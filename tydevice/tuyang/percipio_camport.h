...
//  *** API in this file is NOT Recommended to use in New Proejct ***
//
// ===================================================
// FIXME: TODO: Note:
//   This file is provided only for those old friends
//   who dont want to read our new APIs and are still
//   using our old generation products.
//
// ===================================================
...
enum DeviceProperties {
  ...
  PROP_CALIB_DEPTH_INTRINSIC,   /**< paremeter of camera*/
...
/* *
* @brief struct for depth camera intrinsics parameter
* */
struct CamIntrinsic {
  uint32_t width;   /**< sensor pixel width*/
  uint32_t height;  /**< sensor pixel height*/
  float data[9];    /**< 3x3 intrinsics parameter*/
};
...
class ICoordinateMapper {
 public:
  ...
  virtual const CamIntrinsic* get_depth_intrinsics() = 0;
  /**
...
class ICameraVideoSource {
 public:
  ...
  virtual CameraSourceStatus OpenDevice(const char *ip_address) = 0;
  ...
  virtual TY_DEV_HANDLE get_handle() = 0;
};
...
class DepthCameraDevice {
  ...
  */
  ...
  CameraSourceStatus OpenDevice(const char *ip) {
    return get_source()->OpenDevice(ip);
  }
...
#include "percipio_camport_impl.hpp"
#endif
